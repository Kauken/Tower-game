...
- **`partita_in_corso`** — stato completo della run: ondata, oro, vite, torri piazzate con i loro moduli, reliquie, seme casuale. Sovrascritto a ogni fine ondata. Cancellato a fine partita.
- **`progressione`** — cristalli, potenziamenti permanenti, obiettivi completati, sblocchi, personaggi disponibili, statistiche. **Non va mai perso.** È l'unica cosa davvero preziosa.
...
La run salva il seme del generatore casuale e il numero di estrazioni fatte. Senza, riprendendo una partita cambierebbero i potenziamenti offerti, e il giocatore potrebbe sfruttarlo per ripescare finché non esce quello che vuole.
...
- Non salvare valori derivabili dalla configurazione: salva l'`id` della torre, non le sue statistiche. Altrimenti un ritocco di bilanciamento non arriverebbe mai alle partite salvate.
